...
//inicializa la pestaña y el contenido del mensaje de bienvenida
welcomeTab('Bienvenido.');
switchTab('Asistente%20del%20Empleador');
...
app.post( '/api/message', function(req, res) {
  ...
  //var workspace = 'a63655ca-37c7-4aef-86bc-d62a52bd4da7'
...
function updateMessage(input, response) {
  ...
  if ( !response.output ) {
    ...
    if (response.output.text.indexOf('Muy bien, rellene por favor el formulario de publicación de oferta.') > -1 )
    {
      resetNuevaOferta();//Refresca
      switchTab('Nueva%20Oferta%20de%20Empleo');
    }
        
    if (response.output.text.indexOf('Este es el perfil que hemos encontrado.') > -1)
    {
      resetNuevaOferta();//Refresca
      switchTab('Demandante%20de%20Empleo');
    }
    return response;
  }
...
//*****  funcion para cambiar la pestaña en el nodered dashboard
function switchTab(tabname) {

console.log('new tab:', tabname);
var request = require('request');
request('http://pi-dashboard-alms.mybluemix.net/displayTab?tabname=' + tabname, function (error, response, body) {
...
//*****
//*****  funcion para cambiar el texto de la pestaña de bienvenida
function welcomeTab(myTxt) {
console.log(myTxt);
var request = require('request');
request('http://pi-dashboard-alms.mybluemix.net/welcomeText?txt=' + myTxt, function (error, response, body) {
  if (error) {console.log('error:', error);} // Print the error if one occurred 
  console.log('statusCode:', response && response.statusCode); // Print the response status code if a response was received 
  console.log('body:', body); // Print the HTML for the Google homepage. 
});
}
//*****  funcion reset valores de formulario de oferta
function resetNuevaOferta() {
var request = require('request');
request('http://pi-dashboard-alms.mybluemix.net/resetNuevaOferta', function (error, response, body) {
  if (error) {console.log('error:', error);} // Print the error if one occurred 
  console.log('statusCode:', response && response.statusCode); // Print the response status code if a response was received 
  console.log('body:', body); // Print the HTML for the Google homepage. 
});
